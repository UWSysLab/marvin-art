...
void* RosAlloc::AllocLargeObject(Thread* self, size_t size, size_t* bytes_allocated,
  ...
  *bytes_tl_bulk_allocated = total_bytes;
  ...
  MARVIN_INST_RECORD_ROSALLOC_ALLOC(self, total_bytes, marvin::inst::ROSALLOC_ALLOC_LARGE);
...
inline void* RosAlloc::AllocFromCurrentRunUnlocked(Thread* self, size_t idx) {
  ...
  MARVIN_INST_RECORD_ROSALLOC_ALLOC(self, IndexToBracketSize(idx), marvin::inst::ROSALLOC_ALLOC_NORMAL);
  return slot_addr;
}
...
void* RosAlloc::AllocFromRun(Thread* self, size_t size, size_t* bytes_allocated,
  ...
  if (LIKELY(idx < kNumThreadLocalSizeBrackets)) {
    ...
    *bytes_allocated = bracket_size;
    *usable_size = bracket_size;
    MARVIN_INST_RECORD_ROSALLOC_ALLOC(self, bracket_size, marvin::inst::ROSALLOC_ALLOC_THREAD_LOCAL);
  } else {
  ...
  for (size_t i = 0; i < num_ptrs; i++) {
    ...
    if (kReadPageMapEntryWithoutLockInBulkFree) {
      ...
      if (LIKELY(page_map_entry == kPageMapRun)) {
        ...
        MutexLock mu(self, lock_);
        size_t marvininst_temp_size = FreePages(self, ptr, false);
        freed_bytes += marvininst_temp_size;
        MARVIN_INST_RECORD_ROSALLOC_FREE(self, marvininst_temp_size, marvin::inst::ROSALLOC_FREE_LARGE);
        continue;
      } else {
    ...
    size_t marvininst_temp_size = run->AddToBulkFreeList(ptr);
    freed_bytes += marvininst_temp_size;
    MARVIN_INST_RECORD_ROSALLOC_FREE(self, marvininst_temp_size, marvin::inst::ROSALLOC_FREE_NORMAL_OR_THREAD_LOCAL);
#ifdef __ANDROID__
